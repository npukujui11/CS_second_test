...
* 平衡二叉树：左右子树的高度之差绝对值不超过1的树；平衡二叉树的目的是为了解决二叉排序树因为动态更新导致的性能退化问题，二叉树排序树如果变成链表的话，效率会很差；所谓“平衡”的意思等价于性能不退化，“近似平衡”等价为性能不会退化得太严重
...
* Floyd算法：求任意两点之间的最短路径；
    ...
    + 算法描述：顶点u和v，观察是否存在若干w，使得u->w->v比u->v更短。如果存在，更新路径和距离；
...
### **如何找出出现次数最多的数据？**
* 遍历所有的数据，对数据建立Hash表，key值是数据值，value是次数；
* 如果key值存在，那么value计数值+1；
* 最后遍历hash表，获取最大值，同时获取key值即为次数最多的数据；
...
* 交换排序：冒泡排序，快速排序；
    ...
    + **快速排序**算法：
        ...
        - 快速排序算法的空间复杂度是O(${log}_2n$)，时间复杂度是O(${nlog}_2n$)，快速排序是所有内部排序算法中平均性能最优的排序算法；快速排序属于不稳定算法。
* 选择排序：简单选择排序，堆排序；
    + **简单选择排序**：
